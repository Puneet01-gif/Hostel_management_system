...
class DashboardService:
    ...
    def get_pending_requests(self, force_refresh: bool = False) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            pending_guest_requests = list(GuestRequest.objects.filter(
                status__iexact='pending'
            ).select_related('student').order_by('-created_at')[:10].values(
                'id', 'guest_name', 'start_date', 'end_date', 'created_at',
            ...
            pending_maintenance_requests = list(MaintenanceRequest.objects.filter(
                status__iexact='pending'
            ).select_related('student').order_by('-created_at')[:10].values(
                'id', 'description', 'issue_type', 'priority', 'room_number', 'created_at',
